...
index_considered=201;
c_considered=-0.0027257+0.00010979i;
...
if strcmp(Scanning_method,'wavelength')
 ...
 %deff_1_p=0.8962E-22;
            %write(*,*) 2*((perm/sus)**1.5)*((c/lam_p*1E6)**2)*(d_BPM**2)*(0.0032**2)/(np**3)*power_p/(area(j))*100
             ...
             delta_k_SHG(j)=2*pi*(nc/(lam_c*1E-6)-np/(lam_p*1E-6)-np/(lam_p*1E-6)-index_considered/(QPM_pitch*1E-6));         
    ...
    for q=1:N_2                        %q為position之index   600為一週期
			...
			position=position+dz_2;                   %光束傳遞位置
           
		    M_SHG=i*delta_k_SHG(j)*position;
           
           
		   B=B+dz_2*exp(M_SHG).*c_considered;            %計算倍頻光之產生
	     ...
	     B_sumed=sum(abs(B));                                %明天來討論這行
         SHG_power(j,:)=((A(j)*(abs(B).^2)/(Device_length^2)*(Repetition_rate*Pulsewidth))*area(j));
         
		 position=0;                        %改變波長前須將晶體位置歸零
