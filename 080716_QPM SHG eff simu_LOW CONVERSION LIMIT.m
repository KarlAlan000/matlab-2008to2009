...
%===========調變參數===============
          ...
          QPM_wavelength=1.55;   
          Fundamental_wavelength=1.525:0.0001:1.575;        %micron
          Ratio=1;
...
%gpr_1=Scanning_pitch*(j-1);
            ...
            device_length(j)=number_of_period*gpr_1*1E-6;   %  (m)
...
if strcmp(Material,'LN')
 ...
 %deff_1_p=0.8962E-22;
            %write(*,*) 2*((perm/sus)**1.5)*((c/lam_p*1E6)**2)*(d_BPM**2)*(0.0032**2)/(np**3)*power_p/(area(j))*100
             ...
             A(j)=2*((d_BPM*2/pi*sus)^2)*(fre_p^2)*(power_p^2)/(area(j)^2)*((perm/sus)^(3/2))*(device_length(j)^2)/(np^2)/nc;

	
            delta_k_SHG(j)=2*pi*(nc/(lam_c*1E-6)-np/(lam_p*1E-6)-np/(lam_p*1E-6)-1/(gpr_1*1E-6));   %倍頻相位不匹配程度 這個好像不用是array

           
        B(j)=sin(delta_k_SHG(j)*device_length(j)/2)^2/(delta_k_SHG(j)*device_length(j)/2)^2;
	                                
         SHG_power(j)=(A(j)*B(j)*(Repetition_rate*Pulsewidth))*area(j);
