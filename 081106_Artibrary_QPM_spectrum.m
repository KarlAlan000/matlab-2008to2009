...
Fundamental_wavelength=1.40:0.001:1.7;        %micron
Averaged_power=0.12;                      %平均入射光功率(W)
Repetition_rate=4000;         %Hz
Pulsewidth=27E-9;             %second
QPM_period=19.15;               %micron
cd('D:\TuanShu');
Spatial_resolution=0.1; % (um) 這個數值要跟file 1一致
Averaging_times=1;   %在此m-file中, 此數值決定的是讀取之coeff量
...
elseif strcmp(Material,'SLT')    	
			...
			area(j)=pi*(W3(j)*1E-3)^2;                       %光點面積(m^2)
             ...
             delta_k_SHG=2*pi*(nc/(lam_c*1E-6)-np/(lam_p*1E-6)-np/(lam_p*1E-6)-1/(QPM_period*1E-6));   %倍頻相位不匹配程度 這個是一個array!!!

             A(j)=2*((D_BPM*sus)^2)*(fre_p^2)*(power_p^2)/(area(j)^2)*((perm/sus)^(3/2))*(Device_length^2)/(np^2)/nc;

             B(j)=sum(-2*2/pi./delta_k_SHG.*exp(i*delta_k_SHG*Device_length/2).*sin(delta_k_SHG*Device_length/2));
