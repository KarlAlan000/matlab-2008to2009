...
Fundamental_wavelength=1.50:0.0001:1.6;        %micron
...
elseif strcmp(Material,'SLT')    	
			...
			area(j)=pi*(W3(j)*1E-3)^2;                       %光點面積(m^2)
             ...
             delta_k_SHG=2*pi*(nc/(lam_c*1E-6)-np/(lam_p*1E-6)-np/(lam_p*1E-6));   %倍頻相位不匹配程度 這個是一個array!!!

             A(j)=2*((D_BPM*sus)^2)*(fre_p^2)*(power_p^2)/(area(j)^2)*((perm/sus)^(3/2))*(Device_length^2)/(np^2)/nc;

             B(j)=sum(-2./delta_k_SHG.*exp(i*delta_k_SHG*Device_length/2).*sin(delta_k_SHG*Device_length/2));
