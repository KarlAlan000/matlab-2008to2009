...
if strcmp(Material,'LN')
 ...
 %deff_1_p=0.8962E-22;
            %write(*,*) 2*((perm/sus)**1.5)*((c/lam_p*1E6)**2)*(d_BPM**2)*(0.0032**2)/(np**3)*power_p/(area(j))*100
             ...
             A(j)=2*(d_BPM*sus)^2*fre_p^2*power_p^2/area(j)^2*(perm/sus)^(3/2)/np^2/nc;
    ...
    for q=1:q_p                        %q為position之index   600為一週期
		   ...
		   B(q+1)=B(q)+dz*exp(M_SHG(q))*deff;            %計算倍頻光之產生
         ...
         SHG_power(j)=A(j)*abs(B(q_p+1))^2*(Repetition_rate*Pulsewidth)*area(j);
