...
if strcmp(Scanning_method,'wavelength')
 ...
 %deff_1_p=0.8962E-22;
    ...
    for q=1:N_2                        %q為position之index   600為一週期
            ...
            delta_k_SHG=2*pi*(nc/(lam_c*1E-6)-np/(lam_p*1E-6)-np/(lam_p*1E-6)-index_considered/(QPM_pitch*1E-6));   %倍頻相位不匹配程度 這個是一個array!!!
			

			position=position+dz_2;                   %光束傳遞位置
           
		    M_SHG=i*delta_k_SHG*position;
