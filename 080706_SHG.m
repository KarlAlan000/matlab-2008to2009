...
%===========調變參數===============
          ...
          Scanning_pitch=0.001;     %全部的單位都是micron (in spectral domain)
          Ratio=0.9;
          power_p=0.1;                      %入射光功率(W), 以後這個可能也要scan
          Averaging_times=1;
%=================================

%===========平常不動的參數=========
            q_p=600;                %將每個QPM pitch分成這麼多個pitch (spitially) (QPM pitch/q_p=dz)
            random_pitch=1;        %單位dz, 最好設為q_p/2 (coherent length)的因數, 不然會遇到換poling方向時一個pitch被切斷的情況(物理圖像不對)
            number_of_period=100;      %週期數量, 此數值乘上QPM週期(這是算出來的)即sevice長度
%====================================


%===========預先宣告陣列長度==========
Z01(1:(Scanning_range/Scanning_pitch)+1)=0;
W3(1:(Scanning_range/Scanning_pitch)+1)=0;
Z03(1:(Scanning_range/Scanning_pitch)+1)=0;
area(1:(Scanning_range/Scanning_pitch)+1)=0;
A(1:q_p)=0;
B(1:q_p)=0;
delta_k_SHG(1:q_p)=0;
position(1:q_p)=0;
...
power_run_A(1:q_p)=0;
power_run_B(1:q_p)=0;
SHG_power_temp(1:(Scanning_range/Scanning_pitch)+1)=0;
SHG_wavelength_t(1:(Scanning_range/Scanning_pitch)+1)=0;
Averaging_temp(1:(Scanning_range/Scanning_pitch)+1)=0;
%==================================== 
...
for j=1:(Scanning_range/Scanning_pitch)+1                                
        %write(*,*),j
% lam_p=QPM_wavelength;
        
            lam_p=QPM_wavelength-0.5*Scanning_range+Scanning_pitch*(j-1);   %基頻光波長(um) 是j的函數
...
%		    d_BPM=3.04612E-22;                                                          %不知道這是啥單位 to check FOR LN
    ...
    for q=1:q_p                        %q為position之index   600為一週期
		 ...
		 SHG_power_temp(j)=power_run_B(q_p)';                % (W)
...
for j=1:(Scanning_range/Scanning_pitch)+1                                %生成畫圖用的wavelength scale (micron)
    SHG_wavelength_t(j)=QPM_wavelength-0.5*Scanning_range+Scanning_pitch*(j-1);
% SHG_wavelength_t(j)=Scanning_pitch*(j-1);
end
