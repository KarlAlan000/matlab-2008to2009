...
QPM_wavelength=1.55;    %
Number_of_pitch=250;    % 這兩個必須和之前一致
...
Device_length_coeff=Number_of_pitch*QPM_pitch*1E-6;
Device_length=Device_length_coeff;
...
if strcmp(Scanning_method,'wavelength')
 ...
 %deff_1_p=0.8962E-22;
            %write(*,*) 2*((perm/sus)**1.5)*((c/lam_p*1E6)**2)*(d_BPM**2)*(0.0032**2)/(np**3)*power_p/(area(j))*100
             ...
             delta_k_SHG=2*pi*(nc/(lam_c*1E-6)-np/(lam_p*1E-6)-np/(lam_p*1E-6)-index_considered/(Device_length_coeff));   %倍頻相位不匹配程度 這個是一個array!!!
