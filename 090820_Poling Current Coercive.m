...
R_1=500;
current_R=current-voltage*R_1;
charge=zeros(length(current),1);
charge_R=zeros(length(current_R),1);
for j=1:length(current)
charge(j)=sum(current(1:j)*Reducing_window_index);    %current(nA) original time spacing=msecond -> charge單位為pC
end
for j=1:length(current_R)
charge_R(j)=sum(current_R(1:j)*Reducing_window_index);    %current(nA) original time spacing=msecond -> charge單位為pC
end
charge=charge-0.5*min(charge);
charge_R=charge_R-0.5*min(charge_R);
plot(voltage,charge);
